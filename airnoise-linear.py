#!/usr/bin/env python3
# airnoise-linear.py
# Train linear models (Logistic Regression) for Air Quality Label & Noise Label
...
def train_and_eval(csv_path: str, save_path: str, test_size: float = 0.20, random_state: int = 42):
    ...
    # Pipelines (scaler + logistic regression)
    aq_pipe = Pipeline([
    ...
    report("Air Quality Label (LogReg)", ya_te, y_aq_pred, le_aq.classes_)
    report("Noise Label (LogReg)",      yn_te, y_n_pred,  le_noise.classes_)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--save", default="air-noise-linear.pkl")
    ...
    ap.add_argument("--temp", type=float, help="Temperature (°C)")
    ap.add_argument("--hum", type=float, help="Humidity (%)")
    ap.add_argument("--mq135", type=float, help="MQ135 Value")
    ap.add_argument("--noise", type=float, help="Noise (dB)")
    args = ap.parse_args()
